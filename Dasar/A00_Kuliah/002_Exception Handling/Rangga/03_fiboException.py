...
if __name__ == '__main__':
    for i in range(5):
        print(75 * "=")
        print("{:^75}".format("MENCARI DERET FIBONACCI"))
        print(75 * "=")
        try :
            print(75 * "-")
            key = eval(input("Masukkan Indeks deret yang dicari : "))

        except(SyntaxError,NameError,EOFError,KeyboardInterrupt) as e :
            print()
            print("[!]Error Message\t\t: Masukkan input dengan benar!")
            print("[!]Type Error\t\t\t: {}".format(str(type(e))[8:(len(str(type(e)))-2)]))

        else :
            angka = fibbonacci(key)
            angka.hitung()
            angka.tampilkanDeret()
